...
:: ENVIRONMENT
::
::     The script will automatically create an appropriate firewall rules if the
::     following environment variables are set:
::
::     CLIENT_HTTP
::         Defines the path to the exe of default HTTP Client (such as Firefox).
::
::     CLIENT_MAIL
::         Defines the path to the exe of default Mail Client (such as Outlook).
::
::     CLIENT_SSH
::         Defines the path to the exe of default SSH Client (such as Putty).
::
::     CLIENT_SCP
::         Defines the path to the exe of default SCP Client (such as WinSCP).
::
::     CLIENT_VCS
::         Defines the path to the exe of default VCS Client (such as Git).
::
:: KNOWN ISSUES
::
...
:services
    ...
    for %%i in (^
        ...
        sc query %%i > nul && if errorlevel 0 (
            ...
            sc stop %%i > nul & sc config %%i start= disabled > nul
            reg add "HKLM\SYSTEM\CurrentControlSet\Services\%%i" /v "Start" /t REG_DWORD /d 4 /f > nul
            if errorlevel 0 (echo ok) else (echo error)
        )
    )
...
:schedule
    ...
    for %%i in (^
        ...
        schtasks /query /tn %%i > nul 2>&1 && if errorlevel 0 (
            ...
            echo | set /p="Stopping Schedule\%%~i... "
...
:firewall
    ...
    netsh advfirewall firewall set rule name=all new enable=no > nul
    if errorlevel 0 (echo ok) else (echo error)
    ...
    for %%r in (^
        ...
        netsh advfirewall firewall set rule name=%%r new enable=yes > nul
        if errorlevel 0 (echo ok) else (echo error)
    )
    ...
    for %%r in (^
        ...
        echo | set /p="Allowing Firewall\%%~r... "
        netsh advfirewall firewall set rule name=%%r profile=private new enable=yes > nul
        if errorlevel 0 (echo ok) else (echo error)
    )
    ...
    netsh advfirewall firewall add rule name="Custom Rules - Windows Time" dir=Out protocol=udp program="%SystemRoot%\system32\svchost.exe" remoteip=Any localport=Any remoteport=123 service=W32Time profile=private action=allow > nul
    if errorlevel 0 (echo ok) else (echo error)

    echo | set /p="Allowing Firewall\Custom Rules - Windows Update... "
    netsh advfirewall firewall add rule name="Custom Rules - Windows Update" dir=Out protocol=tcp program="%SystemRoot%\system32\svchost.exe" remoteip=Any localport=Any remoteport=80,443 service=wuauserv profile=private action=allow > nul
    if errorlevel 0 (echo ok) else (echo error)
